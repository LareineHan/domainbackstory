...
async function rdapBootstrap(domain){
 ...
 const boot=await fjson("https://data.iana.org/rdap/dns.json",9000);
 let base=null;
 ...
 if(!base)throw new Error("No RDAP bootstrap endpoint for TLD");
 if(!base.endsWith("/"))base+="/";
 return await fjson(base+"domain/"+encodeURIComponent(domain),10000);
}
async function rdap(domain){
 let x;
 try{x=await rdapBootstrap(domain)}catch(e){x=await fjson("https://rdap.org/domain/"+encodeURIComponent(domain),10000)}
 ...
 for(const en of(x.entities||[])){
   ...
   const ee={};for(const e of(en.events||[])){if(e.eventAction&&e.eventDate&&!ee[e.eventAction])ee[e.eventAction]=e.eventDate}
   entityCreated=ee.registration||entityCreated;entityChanged=ee["last changed"]||ee.changed||entityChanged;
   break;
 }
 return {
   ...
   registrar_entity_created:entityCreated,
   registrar_entity_changed:entityChanged,
...
async function doh(domain,type){
 const x=await fjson(`https://cloudflare-dns.com/dns-query?name=${encodeURIComponent(domain)}&type=${encodeURIComponent(type)}`,8000,{"accept":"application/dns-json"});
 const ans=x.Answer||[];
 return ans.map(a=>{
   let d=String(a.data||"");
   if(type==="MX"){const m=d.match(/^(\d+)\s+(.+)$/);return m?`${m[1]} ${stripDot(m[2])}`:stripDot(d)}
   if(type==="SOA")return d.split(" ").slice(0,2).map(stripDot).join(" ");
   return stripDot(d.replace(/^"|"$/g,""));
 }).filter(Boolean);
}
async function dns(domain){
 const types=["A","AAAA","MX","NS","CNAME","SOA"];
 const out={};
 const res=await Promise.allSettled(types.map(t=>doh(domain,t)));
 types.forEach((t,i)=>out[t]=res[i].status==="fulfilled"?res[i].value:[]);
 return out;
}
...
async function wayback(domain){const x=await fjson("https://archive.org/wayback/available?url="+encodeURIComponent(domain),9000);return{closest:snapObj(x)}}

async function ct(domain){
 const rows=await fjson("https://crt.sh/?q="+encodeURIComponent(domain)+"&output=json&exclude=expired",9000);
 const ids=new Set(),names=new Map();let newest=null;
 for(const r of(Array.isArray(rows)?rows:[])){if(r.id!=null)ids.add(String(r.id));const dt=r.entry_timestamp||r.not_before||null;if(dt&&(!newest||dt>newest))newest=dt;for(let n of String(r.name_value||r.common_name||"").split("\n")){n=n.trim().toLowerCase().replace(/^\*\./,"");if(n)names.set(n,dt)}}
 return{unique_certificates:ids.size||(Array.isArray(rows)?rows.length:0),unique_names:names.size,newest,recent_names:[...names.entries()].sort((a,b)=>String(b[1]||"").localeCompare(String(a[1]||""))).slice(0,20).map(([name,date])=>({name,date}))};
}
...
export default{async fetch(request,env,ctx){
 ...
 if(u.pathname.startsWith("/api/")){
  ...
  try{
   if(u.pathname==="/api/rdap")return json(await cached(`https://cache.domainbackstory/v6/rdap/${d}`,86400,()=>rdap(d),ctx));
   if(u.pathname==="/api/dns")return json(await cached(`https://cache.domainbackstory/v6/dns/${d}`,3600,()=>dns(d),ctx));
   if(u.pathname==="/api/wayback")return json(await cached(`https://cache.domainbackstory/v6/wayback/${d}`,43200,()=>wayback(d),ctx));
   if(u.pathname==="/api/ct")return json(await cached(`https://cache.domainbackstory/v6/ct/${d}`,21600,()=>ct(d),ctx));
