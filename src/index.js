...
async function rdap(domain){
 ...
 throw new Error("All RDAP sources failed — "+errors.join(" | "));
}
...
export default{async fetch(request,env,ctx){const u=new URL(request.url);if(u.pathname.startsWith("/api/")){const d=getDomain(request);if(!d)return json({error:"Invalid domain"},400);try{
 if(u.pathname==="/api/rdap")return json(await cached(`https://cache.domainbackstory/v10/rdap/${d}`,86400,()=>rdap(d),ctx));
 if(u.pathname==="/api/dns")return json(await cached(`https://cache.domainbackstory/v10/dns/${d}`,3600,()=>dns(d),ctx));
 if(u.pathname==="/api/maildns")return json(await cached(`https://cache.domainbackstory/v10/maildns/${d}`,3600,()=>maildns(d),ctx));
 if(u.pathname==="/api/wayback")return json(await cached(`https://cache.domainbackstory/v10/wayback/${d}`,43200,()=>wayback(d),ctx));
 if(u.pathname==="/api/ct")return json(await cached(`https://cache.domainbackstory/v10/ct/${d}`,21600,()=>ct(d),ctx));
