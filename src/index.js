...
async function wayback(domain){
 const x=await fetchJson("https://archive.org/wayback/available?url="+encodeURIComponent(domain),9000);
 return {closest:snapObj(x),method:"availability-api"}
}
export default{async fetch(request,env,ctx){const u=new URL(request.url);if(u.pathname.startsWith("/api/")){const d=getDomain(request);if(!d)return json({error:"Invalid domain"},400);try{if(u.pathname==="/api/rdap")return json(await cachedJson(`https://cache.domainbackstory/rdap/${d}`,86400,()=>rdap(d),ctx));if(u.pathname==="/api/ct"){const mode=u.searchParams.get("mode")==="deep"?"deep":"quick";return json(await cachedJson(`https://cache.domainbackstory/ct/${mode}/${d}`,mode==="deep"?21600:43200,()=>ct(d,mode),ctx))}if(u.pathname==="/api/wayback")return json(await cachedJson(`https://cache.domainbackstory/wayback/${d}`,43200,()=>wayback(d),ctx));return json({error:"API route not found"},404)}catch(e){return json({error:e.name==="AbortError"?"Provider timed out":e.message,retry_after:e.retryAfter||null},502)}}return env.ASSETS.fetch(request)}}