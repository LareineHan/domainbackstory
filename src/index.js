...
async function fjson(url,ms=9000,headers={},attempts=1){
 let lastErr=null;
 for(let i=0;i<attempts;i++){
  const c=new AbortController(),t=setTimeout(()=>c.abort(),ms);
  try{
   const r=await fetch(url,{headers:{"accept":"application/json,text/plain,*/*","user-agent":"DomainBackstory/15.0",...headers},signal:c.signal});
...
async function rdapBootstrapEndpoint(domain){
 ...
 const boot=await fjson("https://data.iana.org/rdap/dns.json",6000,{},1);
...
async function rdap(domain){
 ...
 try{
  ...
  if(endpoint){
   const x=await fjson(endpoint,8000,{},1);
 ...
 try{
  const x=await fjson("https://rdap.org/domain/"+encodeURIComponent(domain),8000,{},1);
...
export default{async fetch(request,env,ctx){const u=new URL(request.url);if(u.pathname.startsWith("/api/")){const d=getDomain(request);if(!d)return json({error:"Invalid domain"},400);try{
 if(u.pathname==="/api/rdap")return json(await cached(`https://cache.domainbackstory/v15/rdap/${d}`,86400,()=>rdap(d),ctx));
 if(u.pathname==="/api/dns")return json(await cached(`https://cache.domainbackstory/v15/dns/${d}`,3600,()=>dns(d),ctx));
 if(u.pathname==="/api/maildns")return json(await cached(`https://cache.domainbackstory/v15/maildns/${d}`,3600,()=>maildns(d),ctx));
 if(u.pathname==="/api/wayback")return json(await cached(`https://cache.domainbackstory/v15/wayback/${d}`,43200,()=>wayback(d),ctx));
 if(u.pathname==="/api/ct")return json(await cached(`https://cache.domainbackstory/v15/ct/${d}`,21600,()=>ct(d),ctx));
